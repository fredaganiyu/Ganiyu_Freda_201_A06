...
[https://github.com/fredaganiyu/Ganiyu_Freda_201_A06](https://github.com/fredaganiyu/Ganiyu_Freda_201_A06)
...
```{r}
# indexing
LJ_pts <- player_info[[2]][[4]]
LJ_pts

# name
LJ_pts2 <- player_info[["LJ"]][["points"]]
LJ_pts2
...
```{r}
pts_round <- sapply(points_per_game, round, 2)
pts_round
...
rounded_mat_rand <- apply(mat_rand, 1:2, round_mat)

print(mat_rand)

print(rounded_mat_rand)
...
```{r}
avg_temps_ <- colMeans(max_temps[, 3:14], na.rm = TRUE)
print(avg_temps_)
...
```{r}
co_temps <- max_temps %>% filter(state == "CO")
oh_temps <- max_temps %>% filter(state == "OH")
va_temps <- max_temps %>% filter(state == "VA")
```
...
```{r}
co_temps$median_annual_temp <- apply(co_temps[, 3:14], 1, median, na.rm = TRUE)


max_median_temp <- max(co_temps$median_annual_temp, na.rm = TRUE)
yr_max_median <- co_temps$year[which.max(co_temps$median_annual_temp)]

min_median_temp <- min(co_temps$median_annual_temp, na.rm = TRUE)
yr_min_median <- co_temps$year[which.min(co_temps$median_annual_temp)]


print(paste0("The maximum median annual temperature of ", max_median_temp, " occurred in ", yr_max_median, " and the minimum median annual temperature of ", min_median_temp, " occurred in ", yr_min_median, "."))
